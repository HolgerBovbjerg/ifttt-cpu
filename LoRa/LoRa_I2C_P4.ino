...
void loop()
{
  ...
  if (DataAvailable > 0) {
    ...
    */
    bitWrite(SlaveSend, 0,(bitRead(array2[0],0)));                                     //Changes the bits from Data receeved data to SlaveSend one bit at the time.
...
void receiveEvent (int howMany)                                                        //This Function is called when Slave receives value from master
  ...
  bbb = 1;                                                                            //sættes til 1 for at fortælle at der er data der skal sendes
  }               
